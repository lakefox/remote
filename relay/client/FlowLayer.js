...
function Socket(ws, id, connectId) {
    ...
    let requests = {};
    let routes = {};
    let handler = async (raw) => {
        let data = JSON.parse(raw.data);
        if (data.id == id && data.type == "data") {
            if (data.channel == undefined) {
                call(data.data.type, data.data.data);
            }
        } else if (data.id == id && data.type == "fetch" && data.status === 1) {
            if (requests[data.data.type]) {
                if (requests[data.data.type][data.index]) {
                    requests[data.data.type][data.index](data.data.data);
                }
            }
            ...
            if (routes[data.data.type]) {
                ...
                // Catch all route for fetch requests
